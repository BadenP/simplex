...
def obter_valores():
    ...
    for i in range(num_variaveis):
        ...
        entrada_valor.grid(row=4, column=count, padx=1, ipadx=1, pady=1)
        
        rotulo_variavel = Label(janela, text=f"X{i+1}"+string)
        rotulo_variavel.grid(row=4, column=count+1, padx=10, pady=1)

        entradas_variaveis_objetivo.append(entrada_valor)
        count += 2

    rotulo_objetivo = Label(janela, text=f"Restrições:")
    rotulo_objetivo.grid(row=5, column=0, padx=10, pady=10)
    ...
    for r in range(num_restricoes):
        ...
        for i in range(num_variaveis):
            ...
            entrada_valor = Entry(janela, width=5)
            entrada_valor.grid(row=5+r, column=count, padx=10, pady=10)
            
            rotulo_variavel = Label(janela, text=f"X{i+1}" + string)
            rotulo_variavel.grid(row=5+r, column=count+1, padx=10, pady=10)
        ...
        entrada_valor2.grid(row=5+r, column=count+2, padx=10, pady=10)
